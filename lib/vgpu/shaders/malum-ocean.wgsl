...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let specTight = blinnSpecular(n, viewDir, lightDir, mix(90.0, 160.0, params.quality));
  let spec = mix(specWide * 0.45, specTight, 0.62) * water;
  let chop = fbmSimplex2d(world * 0.55 + vec2f(params.time * 0.07, 0.0), 2, 2.12, 0.5);
  ...
  let cloud = fbmSimplex2d(uv * 0.42 + vec2f(params.time * 0.0055, params.time * 0.0032), 3, 2.0, 0.55);
