...
fn hushMask(plateUv: vec2f) -> f32 {
  let a = boatAxes(plateUv);
  let e = vec2f((a.x + 0.012) / 0.165, a.y / 0.1);
  return 1.0 - smoothstep(0.55, 1.28, length(e));
}
...
// Slow searchlight swell. Two long sines so it only overexposes when they
// align — no flicker, the scene never goes dark, just less bloom vs more glare.
fn searchlightPulse(t: f32) -> f32 {
  let a = 0.5 + 0.5 * sin(t * 0.29);
  let b = 0.5 + 0.5 * sin(t * 0.17 + 2.15);
  let swell = pow(a * b, 1.45);
  return 0.96 + swell * 0.2;
}
...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let live = 1.0 - hull;
  let water = live * (1.0 - hush * 0.9);
  let pulse = searchlightPulse(params.time);
  let glare = smoothstep(1.06, 1.15, pulse);
  ...
  let lampGain = mix(0.9, pulse, 0.85);
  col += cool * shaft * water * (0.04 + 0.08 * cau * through + 0.2 * spec * over) * lampGain;
  col += vec3f(0.96, 0.98, 1.0) * spec * nDotL * over * shaft * water * 0.24 * lampGain;
  ...
  col = mix(col, max(col, vec3f(0.94, 0.96, 1.0)), lamp * glare * 0.55);
  col = mix(col, col * 0.62 + vec3f(0.78, 0.84, 0.94) * 0.38, hullBody * glare * 0.85);

  col *= mix(1.0, 0.88, deep * (1.0 - shaft * 0.45) * live);
  col += vec3f(0.05, 0.12, 0.22) * deep * through * shaft * cau * water * 0.14 * lampGain;
  ...
  col += vec3f(0.86, 0.92, 1.0) * anamorphicFlare(uv, lampUv, glare) * 0.16;
